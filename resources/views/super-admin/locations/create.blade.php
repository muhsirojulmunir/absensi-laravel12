...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="flex items-center space-x-4">
        ...
        <div>
            <h2 class="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Tambah Lokasi Counter</h2>
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Daftarkan lokasi counter Ramayana baru beserta radius absensi.</p>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden">
        <form action="{{ route('super-admin.locations.store') }}" method="POST" class="p-6 sm:p-8 space-y-6">
            @csrf
            ...
            <div class="space-y-1.5">
                ...
                <input type="text" name="name" value="{{ old('name') }}" required 
                       ...
                       placeholder="Contoh: Ramayana Sidoarjo">
            ...
            <div class="p-5 bg-indigo-50/40 dark:bg-indigo-950/10 border border-indigo-100/50 dark:border-indigo-900/40 rounded-xl space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div class="space-y-1.5">
                        <label class="block text-[11px] font-semibold text-slate-600 dark:text-slate-400 pl-0.5">Latitude</label>
                        <input type="text" id="latitude" name="latitude" value="{{ old('latitude') }}" 
                               ...
                               placeholder="Contoh: -7.4478">
                        @error('latitude') <p class="text-xs text-rose-500 mt-1 pl-0.5">{{ $message }}</p> @enderror
                    </div>
                    <div class="space-y-1.5">
                        <label class="block text-[11px] font-semibold text-slate-600 dark:text-slate-400 pl-0.5">Longitude</label>
                        <input type="text" id="longitude" name="longitude" value="{{ old('longitude') }}" 
                               class="w-full bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-805 rounded-xl px-4 py-2.5 text-xs text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all font-mono" 
                               placeholder="Contoh: 112.7183">
            ...
            <div class="space-y-1.5">
                ...
                <input type="number" name="radius" value="{{ old('radius', 100) }}" required min="1" 
